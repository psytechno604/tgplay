...
module.exports = async function (opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    spawned.on('disconnect', code => {
      opts.logger && this.logger.info(`disconnect: child process exited with code ${code}`, opts)
      codeProcessor({ code, res, opts, resolve, reject })
    })

    spawned.on('exit', code => {
      this.logger.info(`exit: child process exited with code ${code}`, opts)
      codeProcessor({ code, res, opts, resolve, reject })
    })
